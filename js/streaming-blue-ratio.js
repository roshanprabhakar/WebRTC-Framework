...
let initiateConnectionStream = async function () {
    ...
    let packet = [] //a complete packet is merged into a frame and rendered immediately (packets/frames aren't queued)
    ...
    channel.onmessage = function (event) {
        ...
        if (messageIndex === 0) { //the first message received details dimensions for the rendering canvas element
            ...
            // let arrayFrame = postprocess(packet); //returns a boolean indicating completion of frame reconstruction then reconstructed frame
            let arrayFrame = postprocessBinaryIndicator(packet); //returns a boolean indicating completion of frame reconstruction then reconstructed frame
            if (arrayFrame[0]) streamedCanvasContext.putImageData(arrayFrame[1], 0, 0);
        }
        messageIndex++;
    }
...
function transmitLoop() {
    if (localVideo && !localVideo.paused && !localVideo.ended) { //ensures stream is still being generated
        ...
        let imageData = localCanvasContext.getImageData(0, 0, width, height).data

        // let packets = preprocess(imageData); //list of arraybuffers
        let packets = preprocessBinaryIndicator(imageData);
        ...
        setTimeout(transmitLoop, 1000 / 100);
    }
}

//input: Uint8ClampedArray (containing pixel data of the image feed)
//output: Array<ArrayBuffer> (containing list of buffers to be transmitted independently)
function preprocess(imageData) {
    let buffer = imageData.buffer;
    return split(buffer, 6);
}

//input: Array<ArrayBuffer> (containing list of buffers to be merged and processed if complete)
//output: ImageData (ImageData to be displayed on the receiving end)
function postprocess(packetList) {
    if (packetList.length === 7) {
        let frameBuffer = mergeList(packetList); //merge arraybuffers contained in packet into a single buffer depicting an ImageData object skeleton

        //converting the frameBuffer arraybuffer into an ImageData object, after processing
        let arrayFrame = new Uint8ClampedArray(frameBuffer);
        let frame = new ImageData(arrayFrame, width, height);

        packetList.length = 0; //resets packetList if it contains a full frame
...
//input: Uint8ClampedArray (containing pixel data of the image feed)
//output: Array<ArrayBuffer> (containing list of buffers to be transmitted independently)
function preprocessBinaryIndicator(imageData) {
    let counter = 0;
    for (let i = 0; i < imageData.length; i += 4) {
        if (
            Math.abs(imageData[i] - 65) < 20 &&
            Math.abs(imageData[i + 1] - 120) < 20 &&
            Math.abs(imageData[i + 2] - 130) < 20
        ) {
            counter++
        }
    }
    let out = Math.trunc(counter / (imageData.length / 4) * 100);
    console.log(out);
    let container = new Uint8ClampedArray(1);
    container[0] = out;
...
//input: Array<ArrayBuffer> (containing list of buffers to be merged and processed if complete)
//output: ImageData (ImageData to be displayed on the receiving end)
function postprocessBinaryIndicator(packetList) {
    if (packetList.length === 1) {
        let frameBuffer = mergeList(packetList);

        let transmittedReadable = new Uint8ClampedArray(frameBuffer);
        let arrayFrame = new Uint8ClampedArray(4 * width * height);

        for (let i = 0; i < width * height; i += 1) {
            if (i <= arrayFrame.length / 4 * (transmittedReadable[0] / 100)) {
                for (let j = 0; j < 4; j++) {
                    arrayFrame[i * 4 + j] = 255
                }
            } else {
                for (let j = 0; j < 3; j++) {
                    arrayFrame[i * 4 + j] = 0;
                }
                arrayFrame[i * 4 + 3] = 255;
            }
        }

        packetList.length = 0;

        return [true, new ImageData(arrayFrame, width, height)];
    } else return [false, null];
}
...
function split(arraybuffer, n) {
    if (arraybuffer.byteLength < n) return [arraybuffer];
